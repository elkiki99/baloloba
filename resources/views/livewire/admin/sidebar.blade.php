...
<aside class="flex sticky top-0 flex-col space-y-6 pb-[20vh] sm:pt-[12vh] pt-[8vh]">
    <!-- Home -->
    <a wire:navigate href="{{ route('panel') }}" wire:current.strict='text-yellow-600 font-black' class="flex items-center gap-2 hover:blur-xs">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
            class="opacity-50 size-6">
            <path stroke-linecap="round" stroke-linejoin="round"
                d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
        </svg>

        <p class="hidden sm:block">Panel</p>
    </a>

    <!-- Porfolio -->
    <a wire:navigate href="{{ route('photoshoot.index') }}" class="flex items-center gap-2 hover:blur-xs {{ request()->is('photoshoots/*') || request()->is('photoshoots') || request()->is('photoshoot/*') ? 'text-yellow-600 font-black' : '' }}">
        ...
        <p class="hidden sm:block">Portfolio</p>
    </a>
    ...
    <a wire:navigate href="{{ route('packages.index') }}" class="flex items-center gap-2 hover:blur-xs hover:cursor-pointer {{ request()->is('paquetes') || request()->is('paquete/*') ? 'text-yellow-600 font-black' : '' }}">
        ...
        <p class="hidden sm:block">Paquetes</p>
    </a>
    ...
    <a wire:navigate href="{{ route('categories.index') }}" class="flex items-center gap-2 hover:blur-xs {{ request()->is('categorias') || request()->is('categoria/*') ? 'text-yellow-600 font-black' : '' }}">
        ...
        <p class="hidden sm:block">Categorías</p>
    </a>
    ...
    <a class="flex items-center gap-2 hover:blur-xs {{ request()->is('componentes/*') || request()->is('componentes') || request()->is('componente/*') ? 'text-yellow-600 font-black' : '' }}" href="{{ route('components.index') }}" wire:navigate>
        ...
        <p class="hidden sm:block">Componentes</p>
    </a>
