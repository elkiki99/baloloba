...
<div class="max-w-[500px] mx-auto min-h-[50vh] md:min-h-[75vh] flex flex-col my-10 p-6 text-center transition transform md:scale-110 bg-gray-950 border rounded-lg shadow-lg md:w-1/3 md:hover:scale-[1.12]"
    x-data="{
    ...
    <div class="justify-between pb-2 flex text-start">
        <div class="">
            <h2 class="text-xl lg:text-2xl text-white font-semibold">{{ $polaroidsPackage->name }}</h2>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="size-16 lg:size-20 text-white">
        ...
        <div class="flex flex-col text-gray-100 items-end gap-2">
            <span class="text-3xl lg:text-4xl font-bold underline decoration-yellow-500"
                x-text="isExtended ? extendedPrice : basicPrice"></span>
            <span class="text-md lg:text-xl text-gray-500 font-bold line-through" x-text="isExtended ? beforeExtendedPrice : beforeBasicPrice"></span>
        </div>
    </div>
    ...
    <ul class="flex-grow my-4">
        <template x-for="(feature, index) in (isExtended ? extendedFeatures : basicFeatures)" :key="index">
            <li class="flex mb-2 text-gray-200 text-start text-sm lg:text-base" x-text="'✔ ' + feature"></li>
        </template>
    </ul>

    {{-- @guest --}}
        <a :href="isExtended ? annualStructuralUrl : monthlyStructuralUrl"
            class="flex items-center justify-center w-full px-4 py-2 my-2 mt-auto text-gray-900 bg-yellow-500 rounded-lg hover:blur-xs hover:cursor-pointer">Obtener sesión
        </a>
    {{-- @endguest --}}
</div>